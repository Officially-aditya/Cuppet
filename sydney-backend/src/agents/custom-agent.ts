...
export async function renderLlmCustomAgent(input: {
  ...
  try {
    ...
    const system = [
      ...
      "If external data is required (like email, Slack, private documents) that cannot be retrieved via web search, say what connector is needed instead of inventing results.",
