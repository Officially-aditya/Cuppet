...
export function WorkspaceApp({ demo, onExitDemo }: { demo: boolean; onExitDemo?: () => void }) {
  ...
  return <main className={`workspace-shell live-workspace view-${view} ${threadOpen ? "mobile-thread-open" : ""}`}>
    ...
    <nav className="bottom-nav" aria-label="Primary navigation">{navItems.map(({ id, label, iconClass }) => <button key={id} title={label} className={`bottom-nav-item ${view === id ? "active" : ""}`} onClick={() => setView(id)}><span className={`app-nav-icon ${iconClass}`} aria-hidden="true" /><span>{label}</span>{id === "inbox" && agents.reduce((sum, agent) => sum + (agent.unread_count ?? 0), 0) > 0 && <b className="bottom-nav-count">{agents.reduce((sum, agent) => sum + (agent.unread_count ?? 0), 0)}</b>}</button>)}</nav>
