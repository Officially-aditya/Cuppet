...
export const api = {
  ...
  memories: () => apiRequest<{ memories: AssistantMemory[]; compacted_memory?: CompactedMemory | null }>("/users/me/assistant-memories"),
  ...
  deleteCompactedMemory: () => apiRequest<void>("/users/me/assistant-memories/compacted", { method: "DELETE" }),
  archiveState: () => apiRequest<ArchiveState>("/users/me/message-archive"),
  ...
  archivedMessages: (agentId: string, cursor?: string, limit = 50) => apiRequest<ArchivedMessagePage>(`/agents/${agentId}/archived-messages?limit=${Math.min(50, Math.max(1, limit))}${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`),
